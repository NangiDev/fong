...
public class LightSource implements Comparable<LightSource>{
	...
	public LightSource(float x, float y, ShaderManager shaderManager){
		pos = new Vector3((1.0f/ FongMain.screenWidth) * x, (1.0f/ FongMain.screenHeight) * y, defaultLightZ);
		setDefaultLight();
	...
	public void updateLight() {
		setPos((float)(FongMain.screenWidth*(0.5f + Math.sin(time))), (float)(FongMain.screenHeight*(0.5f + Math.cos(time))), defaultLightZ);
		time = time + 0.01f;
	...
	public void setPos(float x, float y, float z){
		this.pos = new Vector3((1.0f/ FongMain.screenWidth)*x, (1.0f/ FongMain.screenHeight)*y, z);
	}
