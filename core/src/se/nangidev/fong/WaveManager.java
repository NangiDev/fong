...
public class WaveManager {
    private final Array<Integer> waveOrder = new Array<Integer>();
    private final FongMain game;
    private final int waveDelay;
    private final ArrayList<se.nangidev.fong.DrawComponent> enemies = new ArrayList<se.nangidev.fong.DrawComponent>();
    private int key = 0;
    ...
    public void tick(float delta) {
        ...
        for (se.nangidev.fong.DrawComponent e : enemies) {
            killWaveBonus = !(e.getY() < Player.getPlayerPos().y);
        }
        ...
        if (waveStartTimer <= 0 && enemies.size() == 0) {
            ...
            waveStart = false;
            waveStartTimer = TimeUtils.nanoTime();
        } else {
            waveStartTimerDiff = (TimeUtils.nanoTime() - waveStartTimer) / 1000000;
