	...
	public void tick(float delta){
		...
		for (int i = 0; i < ticks.size(); i++) {
			for (int j = i+1; j < ticks.size(); j++) {
				if(ticks.get(i) instanceof CollidableComponent && ticks.get(j) instanceof CollidableComponent){
					if(((CollidableComponent)ticks.get(i)).intersectsWith((CollidableComponent)ticks.get(j))){
						CollidableComponent a = (CollidableComponent)ticks.get(i);
